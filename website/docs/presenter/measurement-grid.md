...
| Element | Meaning |
| --- | --- |
| Cyan rectangle | the true bounds of the scaled `1280x720` stage |
| Magenta crosshair | the stage's horizontal + vertical centre lines |
| `stage 1280x720 -> 1742x980` (top-left) | logical size -> rendered size at the current scale |
| `scaled 1742x980 (16:9)` (bottom-right) | the rendered dimensions and aspect confirmation |
| `1840x980 | x1.361` (header) | the viewport size and the scale factor in use |
...
- The header's readout (`x1.361`) must match the stage readout's scale -> confirms the header and slide share one coordinate space.
