...
| Layer | What | z-index |
| --- | --- | --- |
| **1 - Presentation** | the actual slide, in a `1280x720` `<iframe>`, top-anchored and scaled to the viewport | base |
...
Slides are authored at a fixed `1280x720`. The presenter computes a single scale factor and applies it with one transform:
...
Each deck slide is a complete HTML document that brings its own `clippyslide.css`. Loading it into an iframe means the presenter renders the **exact** ClippyFlow slide file with zero risk of style bleed between the presenter chrome and the slide.
