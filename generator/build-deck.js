/* ════════════════════════════════════════════════════════════════════════
   ...
   Data-driven generator for the full 24-slide CoE deck in the ClippySlide system.
   One content model + archetype renderers -> slides/deck/NN-name.html (link clippyslide.css).
...
<title>CoE ${n}</title><link rel="stylesheet" href="../clippyslide.css">
<style>html,body{margin:0;background:#000}
.pg{position:absolute;right:22px;bottom:14px;font-size:13px;color:var(--cs-fg-dim)}
.kick{font-weight:700;font-size:26px;letter-spacing:1px;
  background:var(--cs-grad-title);-webkit-background-clip:text;background-clip:text;color:transparent;width:max-content}
...
.ln{font-size:21px;line-height:1.3;color:var(--cs-fg-soft)}
.foot{position:absolute;left:56px;bottom:42px}
.foot .rule{width:392px;height:2px;border-radius:2px;background:var(--cs-grad-blue);margin-bottom:8px}
.foot .txt{font-weight:700;font-size:21px;background:var(--cs-grad-title);
...
.shot .cap{color:var(--cs-fg-dim);font-size:15px;letter-spacing:.4px;text-transform:uppercase}
${extraCss}</style></head><body>
<div class="cs-stage ${stageClass}">${body}<div class="pg">${n} / 24</div></div></body></html>`;
}

/* ── archetypes ── */
function title({n, kicker, sub, main}) {
  const body = `<div class="cs-accent-left" style="top:300px;height:150px"></div>
...
function feature({n, kicker, title, sub, lines, shotCap = 'Product UI', status}) {
  ...
  return page(n, 'cs-stage--glow', body);
}

function guidance({n, title, lines, panelTitle, mono}) {
  const ls = lines.map(l => `<div class="ln" style="max-width:480px">${esc(l)}</div>`).join('');
  const ms = mono.map(m => `<div style="font-family:'Cascadia Code',Consolas,monospace;font-size:13px;line-height:1.5;color:${m.startsWith('#')?'var(--cs-cyan)':'var(--cs-fg-soft)'}">${esc(m)||'&nbsp;'}</div>`).join('');
  const body = `<div class="cs-accent-left" style="top:250px;height:150px"></div>
  <div style="position:absolute;left:56px;top:64px"><div class="big" style="font-size:46px">${esc(title)}</div>
   <div class="lines">${ls}</div></div>
  <div class="cs-panel blue" style="left:640px;top:48px;width:586px;height:624px">
   <div class="cs-in" style="padding:22px 26px;overflow:hidden">
    <div class="cs-h2 under" style="margin-bottom:14px">${esc(panelTitle)}</div>${ms}</div></div>`;
  return page(n, 'cs-stage--glow', body);
}

function panelGrid({n, title, hero, cols}) {
  // cols: array of columns, each array of {label, hero?}
  let html = `<div class="cs-panel hero" style="left:14px;top:14px;width:1252px;height:120px">
    <div class="cs-in"><div class="cs-h2 under" style="position:absolute;left:20px;top:16px;font-size:24px">${esc(hero)}</div></div></div>`;
  ...
  cols.forEach((col, ci) => {
    ...
    col.forEach((cell, ri) => {
      ...
      html += `<div class="cs-panel ${cls}" style="left:${x}px;top:${y}px;width:${colW}px;height:${rowH}px">
        <div class="cs-in"><div class="cs-label" style="position:absolute;left:16px;top:13px">${esc(cell.label)}</div></div></div>`;
    });
  });
  return page(n, '', html);
}
...
function diagram({n, title, pills, tiles, sideTitle, sideItems, sideKind}) {
  const pillH = pills.map((p,i)=>`<div class="cs-pill ${p.k}">${esc(p.t)}</div>`).join('');
  const tileH = tiles.map(t=>`<div class="cs-tile">${esc(t)}</div>`).join('');
  let side = '';
  if (sideKind === 'green') {
    side = `<div class="cs-greenpanel" style="left:924px;top:150px;width:306px;height:430px">
      ${sideItems.map(s=>`<div class="cs-body" style="color:#fff;font-size:18px">${esc(s)}</div>`).join('')}</div>`;
  } else {
    side = `<div style="position:absolute;left:924px;top:150px;width:306px;display:flex;flex-direction:column;gap:14px">
      ${sideItems.map(s=>`<div class="cs-agentcard"><span class="ico" style="background:linear-gradient(135deg,#CA5BCD,#818EFF)"></span>${esc(s)}</div>`).join('')}</div>`;
  }
  const body = `<div class="cs-accent-left" style="top:300px;height:150px"></div>
  <div class="cs-panel blue" style="left:40px;top:64px;width:840px;height:596px"><div class="cs-in" style="padding:22px 26px">
    <div style="font-weight:800;font-size:40px;line-height:1;margin-bottom:16px;background:var(--cs-grad-title);-webkit-background-clip:text;background-clip:text;color:transparent;width:max-content">${esc(title)}</div>
    <div style="position:relative;width:100%;height:466px;background:#fff;border-radius:12px;overflow:hidden">
     ...
     <div style="position:absolute;inset:16px 18px 16px 56px;display:flex;flex-direction:column">
      ...
      <div style="display:flex;align-items:center;gap:10px;margin-top:14px;color:#14161D;font-size:17px">Knowledge<span class="cs-divider" style="flex:1"></span>Tools<span class="cs-divider" style="flex:2"></span></div>
      <div style="flex:1;display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-top:14px">${tileH}</div>
     </div></div></div></div>
  <div style="position:absolute;left:930px;top:108px;font-weight:800;font-size:26px;background:var(--cs-grad-title);-webkit-background-clip:text;background-clip:text;color:transparent">${esc(sideTitle)}</div>
  ${side}`;
  return page(n, 'cs-stage--glow', body);
}
...
function lifecycle({n, title, note, columns, health}) {
  ...
  columns.forEach(col => {
    ...
    col.cards.forEach(c => {
      cols += `<div class="cs-env ${c.k}" style="position:relative;width:${col.w||386}px;height:${col.h||72}px"><span class="name">${esc(c.name)}</span><span class="ver">${esc(c.ver)}</span></div>`;
    });
    cols += `</div>`;
  });
  const body = `<div style="position:absolute;left:24px;top:16px;font-weight:700;font-size:36px;color:#fff">${esc(title)}</div>
  ${note?`<div style="position:absolute;left:24px;top:96px;width:360px;font-size:18px;color:var(--cs-fg-soft)">${esc(note)}</div>`:''}
  ${cols}<div class="cs-health"><span class="lbl">${esc(health)}</span></div>`;
  return page(n, 'cs-stage--glow', body);
}

/* health/validation 3×3 matrix (Environment · Solution · Validation across Dev→Sandbox→Prod) */
function healthmatrix({n, title, note, cols, rows, health}) {
  ...
  cols.forEach((c, ci) => {
    ...
    head += `<div style="position:absolute;left:${x}px;top:${gy0-44}px;width:${colW}px;font-weight:700;font-size:18px;letter-spacing:.8px;color:var(--cs-cyan);text-transform:uppercase">${esc(c)}</div>`;
  });
  let grid = '';
  rows.forEach((r, ri) => {
    ...
    r.cells.forEach((cell, ci) => {
      const x = gx0 + ci * (colW + colGap);
      grid += `<div class="cs-env ${r.env}" style="left:${x}px;top:${y}px;width:${colW}px;height:${rowH}px;padding:18px 22px;gap:4px">
  ...
  ${note?`<div style="position:absolute;left:40px;top:86px;width:760px;font-size:18px;color:var(--cs-fg-soft)">${esc(note)}</div>`:''}
  ${head}${grid}<div class="cs-health"><span class="lbl">${esc(health)}</span></div>`;
  return page(n, 'cs-stage--glow', body);
}
...
function checklist({n, title, sub, groups}) {
  ...
  groups.forEach((g, gi) => {
    ...
    const items = g.items.map((it,ii)=>`<div style="display:flex;gap:10px;font-size:18px;color:var(--cs-fg-soft);margin-bottom:11px"><span style="color:var(--cs-cyan);font-weight:700">${ii+1}.</span>${esc(it)}</div>`).join('');
    cols += `<div class="cs-panel blue" style="left:${x}px;top:170px;width:${colW}px;height:480px"><div class="cs-in" style="padding:20px 24px">
      <div class="cs-h2 under" style="margin-bottom:16px">${esc(g.head)}</div>${items}</div></div>`;
  });
  const body = `<div class="cs-accent-left" style="top:60px;height:80px"></div>
  <div style="position:absolute;left:40px;top:48px"><div class="big" style="font-size:44px">${esc(title)}</div>
   ${sub?`<div class="cs-body" style="font-size:18px;margin-top:6px">${esc(sub)}</div>`:''}</div>${cols}`;
  return page(n, 'cs-stage--glow', body);
}
...
function demo({n, title, sub}) {
  ...
  <div style="position:absolute;left:0;right:0;top:70px;text-align:center">
   ...
   ${sub?`<div class="sub" style="text-align:center;color:var(--cs-fg-soft)">${esc(sub)}</div>`:''}</div>
...
function cta({n, title, sub, body, outcome, locations}) {
  const locs = locations.map(l=>`<div style="margin-bottom:9px"><div style="font-size:15px;color:#fff;font-weight:600">${esc(l.place)}</div><div style="font-size:13px;color:var(--cs-fg-dim)"><i>${esc(l.date)}</i></div></div>`).join('');
  const bd = body.map(p=>`<div class="cs-body" style="font-size:17px;margin-bottom:12px;max-width:430px">${esc(p)}</div>`).join('');
  const html = `<div class="cs-accent-left" style="top:60px;height:90px"></div>
  <div style="position:absolute;left:48px;top:44px;width:840px">
   <div class="big" style="font-size:40px;color:var(--cs-cyan)">${esc(title)}</div>
   <div class="sub" style="font-size:22px;margin-top:6px">${esc(sub)}</div></div>
  <div class="cs-tile" style="position:absolute;left:48px;top:170px;width:560px;height:42px;justify-content:flex-start;padding-left:16px;font-weight:600"><b>Outcome:</b>&nbsp;${esc(outcome)}</div>
  ...
  <div style="position:absolute;left:980px;top:150px;text-align:center;width:250px">
   ...
   <div style="font-size:14px;color:var(--cs-cyan)"><i>aka.ms/CopilotStudioBootcamp</i></div>
   <div class="shot" style="position:relative;margin:14px auto 0;width:150px;height:150px"><span class="cap">QR</span></div></div>
  <div class="cs-panel blue" style="left:980px;top:336px;width:250px;height:300px"><div class="cs-in" style="padding:14px 16px">
   <div class="cs-h2 under" style="font-size:16px;margin-bottom:12px">Proposed Locations / Dates</div>${locs}</div></div>`;
  return page(n, 'cs-stage--glow', html);
}
...
const SLIDES = [
 ...
 // slide 7 = bespoke hand-built (slides/07-orchestration.html); included in index, not regenerated.
...
fs.writeFileSync(path.join(OUT, 'index.json'), JSON.stringify({
  title: 'CoE Framework — ClippySlide', description: 'Oceaneering CoE deck reproduced in the ClippySlide design system.',
  version: '1.0.0', theme: 'clippyslide', generated: new Date().toISOString().slice(0,10),
  slides: SLIDES.map(s => ({ n: s.n, file: `${String(s.n).padStart(2,'0')}.html`, archetype: s.f.name })).concat([{n:7,file:'../slides/07-orchestration.html',archetype:'diagram (bespoke)'}]).sort((a,b)=>a.n-b.n)
}, null, 2));
