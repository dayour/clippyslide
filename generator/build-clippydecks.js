...
const CLIPPYFLOW_CSS = fs.readFileSync(path.join(ROOT, 'design-system', 'clippyflow.css'), 'utf8');
...
const CLIPPYFLOW_VIEWER_CSS = `${CLIPPYFLOW_CSS}
:root{
  color-scheme:dark;
  --cp-bg:var(--cf-navy-deep);--cp-bg-elevated:var(--cf-navy);
  --cp-surface:var(--cf-surface);--cp-surface-soft:var(--cf-surface-2);
  --cp-border:var(--nitrous-tint);--cp-border-strong:var(--nitrous-rim);
  --cp-text:var(--cf-fg);--cp-text-muted:var(--cf-fg-soft);--cp-text-soft:var(--cf-fg-soft);
  --cp-accent:var(--clippy-cyan-br);--cp-accent-hover:var(--clippy-cyan-br);
  --cp-accent-soft:var(--cf-surface-2);--cp-accent-fg:var(--cf-navy-deep);
  --cp-shadow:var(--cf-glow-nitrous);--cp-overlay:var(--cf-surface);
  --cp-panel:var(--cf-surface);--cp-panel-strong:var(--cf-surface);
}`;
...
.planner-sub{font-size:19px;line-height:1.45;max-width:1080px;margin:0}
.planner-content{display:flex;flex-direction:column;flex:1;min-height:0;gap:18px}
.planner-grid{display:grid;gap:18px;flex:1;min-height:0}
.cols2{grid-template-columns:repeat(2,minmax(0,1fr))}.cols3{grid-template-columns:repeat(3,minmax(0,1fr))}
.planner-card{position:relative;inset:auto;display:flex;flex-direction:column;gap:16px;padding:24px;min-width:0}
.planner-card .cf-chip{align-self:flex-start;font-family:var(--cf-mono);color:var(--clippy-cyan-br)}
.planner-card h2{font-size:23px;line-height:1.25;margin:0;color:var(--cf-fg)}
.planner-card h2.fn{font-family:var(--cf-mono);font-size:19px;overflow-wrap:anywhere}
.planner-card p,.planner-card li{font-size:18px;line-height:1.5;color:var(--cf-fg-soft);margin:0}
.planner-card ul{padding-left:20px;margin:0}.planner-card li+li{margin-top:12px}
.planner-card li::marker{color:var(--nitrous)}
.planner-card .sequence{font-family:var(--cf-mono);font-size:13px;color:var(--cf-fg-soft)}
.planner-card .route{font-family:var(--cf-mono);font-size:34px;font-weight:700;line-height:1}
.route.cyan{color:var(--clippy-cyan-br)}.route.purple{color:var(--clippy-purple)}.route.green{color:var(--clippy-green-br)}
.planner-card .cf-ml{margin-top:auto;color:var(--cf-fg-soft)}
.planner-result{min-height:54px;height:auto;padding:14px 20px;flex:0 0 auto}
.planner-result strong{color:var(--clippy-cyan-br);font-weight:600}
.planner-result span{color:var(--cf-fg-soft)}
.planner-flow{display:grid;grid-template-columns:minmax(0,1fr) 26px minmax(0,1fr) 26px minmax(0,1fr);gap:12px;flex:1}
.planner-arrow{align-self:center;width:26px;height:26px;color:var(--nitrous)}
.planner-step{width:44px;height:44px;border-radius:10px;display:grid;place-items:center;border:1px solid var(--nitrous-rim);background:var(--cf-surface-2);font:600 20px var(--cf-mono);color:var(--clippy-cyan-br)}
.planner-table-panel{position:relative;inset:auto;padding:0;overflow:hidden}
.planner-table{width:100%;border-collapse:collapse}
.planner-table th,.planner-table td{text-align:left;padding:14px 18px;border-bottom:1px solid var(--nitrous-tint)}
.planner-table th{font:600 12px var(--cf-mono);letter-spacing:1.3px;text-transform:uppercase;color:var(--clippy-cyan-br);background:var(--cf-surface-2)}
.planner-table td{font-size:16px;line-height:1.4;color:var(--cf-fg-soft)}
.planner-table tr:last-child td{border-bottom:0}
code{font-family:var(--cf-mono);font-size:.88em;color:var(--clippy-cyan-br)}
.planner-foot{display:flex;align-items:center;justify-content:space-between;gap:20px;flex:0 0 auto}
.planner-foot .cf-prompt{font-size:13px}.planner-foot .cf-ml{font-size:10px;color:var(--cf-fg-soft)}
.planner-hero{display:grid;grid-template-columns:minmax(0,1.55fr) minmax(0,1fr);gap:44px;align-items:center;flex:1;min-height:0}
.planner-hero .planner-title{font-size:58px;margin:22px 0;line-height:1.06;width:auto;text-wrap:balance}
.planner-hero .planner-sub{font-size:21px}
.planner-hero-flow{display:flex;flex-direction:column;gap:12px}
.planner-hero-flow .planner-card{padding:20px;gap:10px}
.planner-hero-flow .planner-arrow{transform:rotate(90deg)}
.planner-hero-flow .cf-ml{margin:0}
...
function cfSlide({ n, total, hero = false, kicker, title, subtitle, body, command }) {
  const head = `<div class="planner-head"><span class="cf-mk">${kicker}</span><h1 class="cf-title planner-title">${title}</h1><p class="cf-body planner-sub">${subtitle}</p></div>`;
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${title} - Task Planning</title>
<link rel="stylesheet" href="../design-system/clippyflow.css">
<link rel="stylesheet" href="../design-system/clippyflow-terminal.css">
<style>${PLANNER_CSS}</style></head><body>
<section class="cf-stage cf-scan${hero ? ' cf-stage--hero' : ''}" aria-label="Slide ${n} of ${total}">
<div class="cf-win">
<div class="cf-win-bar"><span class="cf-win-dots" aria-hidden="true"><i></i><i></i><i></i></span><span class="cf-win-title"><b>planning</b> --${hero ? 'overview' : 'slide ' + String(n).padStart(2, '0')}</span><span class="cf-win-meta"><span>${String(n).padStart(2, '0')} / ${String(total).padStart(2, '0')}</span><span class="on">ClippyFlow</span></span></div>
...
function plannerSlides() {
  const T = 8;
  return [
    cfSlide({n:1,total:T,hero:true,kicker:'Planning system',title:'Task Planning Flow Architecture',subtitle:'Generic planning paths from intake to an approved task plan, with stable tile and card identifiers.',command:'plan --theme clippyflow --output approved',body:`<div class="planner-hero-flow">
<div class="cf-panel planner-card"><span class="cf-ml cyan">01 / Intake</span><h2>One request</h2><p>Capture intent and context.</p></div>
${FLOW_ARROW}
<div class="cf-panel planner-card"><span class="cf-ml cyan">02 / Planning</span><h2>Three paths</h2><p>Clarify, guide, or plan directly.</p></div>
${FLOW_ARROW}
<div class="cf-panel planner-card"><span class="cf-ml cyan">03 / Approval</span><h2>One approved plan</h2><p>Review, approve, and start.</p></div></div>`}),
    cfSlide({n:2,total:T,kicker:'Overview',title:'Three routes, one governed output',subtitle:'Every request resolves into the same approved plan, no matter how it enters.',command:'route --paths A,B,C --output task-plan',body:`<div class="planner-grid cols3">
<div class="cf-panel planner-card"><span class="route cyan">A</span><span class="cf-chip">Path A</span><h2>Scope ambiguous</h2><p>Disambiguate the intended deliverable, or guide the user through decision support.</p><span class="cf-ml">Clarify the scope</span></div>
<div class="cf-panel planner-card"><span class="route purple">B</span><span class="cf-chip">Path B</span><h2>Guided intake</h2><p>Use a compact wizard or an open-ended planning interview.</p><span class="cf-ml">Find the right depth</span></div>
<div class="cf-panel planner-card"><span class="route green">C</span><span class="cf-chip">Path C</span><h2>Clear request</h2><p>Move directly into objective, constraints, and success criteria.</p><span class="cf-ml">Plan directly</span></div></div>
<div class="cf-agentcard planner-result"><strong>Shared output</strong><span>Draft a task plan, review it, then approve execution.</span></div>`}),
    cfSlide({n:3,total:T,kicker:'Entry and routing',title:'Classify before asking',subtitle:'Every request enters through the same clarity and context gate.',command:'classify --clarity --context --complexity',body:`<div class="planner-flow">
<div class="cf-panel planner-card"><div class="planner-step">1</div><span class="cf-chip">e1</span><h2 class="fn">capturePlanningRequest()</h2><p>Accept a task, project, or work request.</p><span class="cf-ml">Capture</span></div>
${FLOW_ARROW}
<div class="cf-panel planner-card"><div class="planner-step">2</div><span class="cf-chip">e2</span><h2 class="fn">classifyPlanningIntent()</h2><p>Score clarity, context, complexity, and guided-intake need.</p><span class="cf-ml">Classify</span></div>
${FLOW_ARROW}
<div class="cf-panel planner-card"><div class="planner-step">3</div><span class="cf-chip">e3</span><h2>Decision gate</h2><p>Route to Path A, B, or C.</p><span class="cf-ml">Route</span></div></div>`}),
    cfSlide({n:4,total:T,kicker:'Path A',title:'Resolve ambiguous scope',subtitle:'Offer a deliverable choice, then continue planning or launch decision support.',command:'clarify --lane A1 --or decision-support',body:`<div class="planner-grid cols2">
<div class="cf-panel planner-card"><span class="cf-chip">A1 / Deliverable selected</span><h2>User picks a deliverable shape</h2><div class="sequence">a0 / a1t1 / a1t2 / a1t3</div><ul><li>Confirm the selected planning lane.</li><li>Ask remaining core questions.</li><li>Rollups: <code>a1:q2/3</code>, <code>a1:q3/3</code>, or <code>a1c:q2/2</code>.</li><li>Draft the task plan.</li></ul></div>
<div class="cf-panel planner-card"><span class="cf-chip">A2 / Decision support</span><h2>Help me decide</h2><div class="sequence">a2t1 to a2t4</div><ul><li>Start a short decision-support sequence.</li><li>Collect <code>a2d:q1/n</code>, <code>a2d:q2/n</code>, and so on.</li><li>Reset into <code>a2p:q1/3</code> through <code>a2p:q3/3</code>.</li><li>Draft the task plan.</li></ul></div></div>`}),
    cfSlide({n:5,total:T,kicker:'Path B',title:'Choose the right intake depth',subtitle:'Guided intake supports both compact clarification and adaptive interviewing.',command:'intake --mode compact|interview',body:`<div class="planner-grid cols2">
<div class="cf-panel planner-card"><span class="cf-chip">B1 / Compact</span><h2>Implicit guided intake</h2><div class="sequence">b1t1 to b1t3</div><ul><li>Run a mini intake wizard.</li><li>Ask <code>b1:q1/n</code>, <code>b1:q2/n</code>, and so on.</li><li>Draft after required details are complete.</li></ul></div>
<div class="cf-panel planner-card"><span class="cf-chip">B2 / Adaptive</span><h2>Explicit &quot;Plan with me&quot;</h2><div class="sequence">b2t1 to b2t5</div><ul><li>Start an open-ended planning interview.</li><li>Generate dynamic follow-ups without a visible counter.</li><li>Capture task inputs, constraints, resources, and dependencies.</li><li>Draft the final plan.</li></ul></div></div>`}),
    cfSlide({n:6,total:T,kicker:'Direct and alternate entry',title:'Reuse the same planning engine',subtitle:'Clear requests skip disambiguation; planning tiles can preselect the lane.',command:'plan --direct C1 --or preset D',body:`<div class="planner-grid cols2">
<div class="cf-panel planner-card"><span class="cf-chip">C1 / Direct</span><h2>Direct planning lane</h2><div class="sequence">c1t1 to c1t3</div><p>Ask three standard questions covering objective, constraints, and success criteria: <code>c1:q1/3</code>, <code>c1:q2/3</code>, <code>c1:q3/3</code>.</p><span class="cf-ml">Objective / Constraints / Success</span></div>
<div class="cf-panel planner-card"><span class="cf-chip">D / Alternate entry</span><h2>Planning tile click</h2><div class="sequence">d1 to d4</div><p>A guided-plan tile starts B2 directly. Other preset templates prefill the brief and reuse <code>e1</code>, <code>e2</code>, and <code>e3</code>.</p><span class="cf-ml">Prefill / Classify / Route</span></div></div>`}),
    cfSlide({n:7,total:T,kicker:'Shared terminal',title:'Generate, review, approve',subtitle:'All paths converge on one consistent plan lifecycle.',command:'task-plan --generate --review --approve',body:`<div class="planner-flow">
<div class="cf-panel planner-card"><div class="planner-step">1</div><span class="cf-chip">g1</span><h2 class="fn">generateTaskPlan()</h2><p>Synthesize work breakdown, assumptions, and next actions.</p><span class="cf-ml">Generate</span></div>
${FLOW_ARROW}
<div class="cf-panel planner-card"><div class="planner-step">2</div><span class="cf-chip">g2</span><h2>Show plan card</h2><p>Present approve, revise, or reject actions.</p><span class="cf-ml">Review</span></div>
${FLOW_ARROW}
<div class="cf-panel planner-card"><div class="planner-step">3</div><span class="cf-chip">g3</span><h2>Approve and start</h2><p>Begin execution, export the plan, or open the delivery workspace.</p><span class="cf-ml">Execute</span></div></div>`}),
    cfSlide({n:8,total:T,kicker:'Schema reference',title:'Stable identifiers and rollups',subtitle:'Machine-readable IDs connect visual tiles to question-card sequences.',command:'inspect --step-id --card-rollup',body:`<div class="cf-panel planner-table-panel"><table class="planner-table" aria-label="Planning identifier schema"><thead><tr><th scope="col">Pattern</th><th scope="col">Meaning</th><th scope="col">Examples</th></tr></thead><tbody>
...
<tr><td><code>data-card-rollup</code></td><td>Cards represented by a tile</td><td>Comma-separated card IDs</td></tr></tbody></table></div>`}),
  ];
}
...
function buildAllDecks() {
  ...
  for (const [id, title] of [['slides-clippyflow', 'ClippyFlow Sample Slides'], ['slides', 'TileSlide Sample Slides']]) {
  ...
  counts['planner-card-math'] = buildPlannerDeck();
  writeLibraryDoc(counts);
}

if (require.main === module) {
  const args = process.argv.slice(2);
  if (args.length === 0) buildAllDecks();
  else if (args.length === 1 && args[0] === '--planner-only') buildPlannerDeck();
  else throw new Error('Usage: node generator/build-clippydecks.js [--planner-only]');
}

module.exports = { plannerSlides, presentation, inlineHtml };
